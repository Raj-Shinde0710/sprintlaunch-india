...
export function SprintFinanceSection() {
  const [cash, setCash] = useState(50000);
  const [marketing, setMarketing] = useState(2000);
  const [salaries, setSalaries] = useState(4000);
  const [tools, setTools] = useState(500);

  const monthlyExpenses = marketing + salaries + tools;
  const netBurn = monthlyExpenses;
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i <= Math.min(runwayMonths + 2, 24); i++) {
      data.push({ month: `M${i}`, balance: Math.max(0, balance) });
      balance -= netBurn;
    }
    return data;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-display font-bold">Finance & Runway</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-red-600">${netBurn.toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold">{runwayMonths === Infinity ? "∞" : `${runwayMonths} mo`}</p>
          </CardContent>
        </Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={chartData}>
                  ...
                  <Area type="monotone" dataKey="balance" stroke="hsl(var(--primary))" fill="hsl(var(--primary))" fillOpacity={0.15} strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input type="number" value={salaries} onChange={(e) => setSalaries(Number(e.target.value))} />
            </div>
            <div>
              <Label className="text-xs">Tools & Infra ($)</Label>
              <Input type="number" value={tools} onChange={(e) => setTools(Number(e.target.value))} />
            </div>
            <p className="text-xs text-muted-foreground">💡 Adjust values to see how hiring or cutting costs affects your runway</p>
          </CardContent>
        </Card>
      </div>
    </div>
